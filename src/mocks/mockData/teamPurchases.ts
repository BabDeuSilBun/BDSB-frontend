...
export const teamPurchases: { [meetingId: number]: TeamPurchaseType[] } = {
  1: [
    {
      totalFee: 67000,
      items: [
...
} = Object.keys(teamPurchases).reduce(
  (acc, meetingIdStr) => {
    ...
    const paginatedItems = Array.from({ length: totalPages }, (_, index) => {
      ...
      const teamPurchasePage: TeamPurchaseType = {
        totalFee: purchases.reduce(
          (total, purchase) => total + purchase.totalFee,
          0,
        ),
