...
const MeetingInfo: React.FC<MeetingInfoProps> = ({ meeting }) => (
  <InfoContainer>
    <InfoRow>
      ...
      <InfoDescription
        aria-label={`Delivery time: ${meeting?.isEarlyPaymentAvailable ? '바로 주문' : '예약 주문'}`}
      >
    ...
    <InfoRow>
      ...
      <InfoDescription
        aria-label={`Meeting location: ${meeting?.metAddress?.metStreetAddress} ${meeting?.metAddress?.metDetailAddress}`}
      >
    ...
    <InfoRow>
      ...
      <InfoDescription
        aria-label={`Delivery fee: ${meeting?.deliveryFeeRange}`}
      >
